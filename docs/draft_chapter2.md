...
在本节中，我们将探讨这些对列表和元组的基本操作，包括插入、删除、搜索和遍历。插入的两种方法包括insert 和 append。插入操作允许您在列表的开头、中间或末尾添加新元素。以下示例演示如何在列表的开头、中间和末尾插入一个元素：
...
删除的两种方法包括 pop 和 remove。删除操作允许您从列表中移除元素。以下示例演示如何删除一个元素：
...
**字符串分割(split)**
Python 中的 `split()` 函数是一个字符串方法，根据指定的分隔符将字符串拆分成子串列表。语法为：string.split(分隔符, maxsplit)
...
* `\w` : 匹配任何字符
* `\W` : 匹配任何非字符
* `\b` : 匹配字母边界  **b=border**  \b代表匹配字符在单词的前面
* `\s` : 匹配空格
* `\S` : 匹配非空格
* `$` : 匹配句子的结尾
...
* [ ]是用来规定匹配范围的
* { }的作用是规定匹配的数量,\d{1,4}匹配一个或者4个连在一起的数字,\d{,4}匹配小于等于四个的数字
...
match = re.search(pattern, text)  ###match.group(0)表示第一个匹配到的实例,依次类推
match1 = re.search(pattern1, text)
if match:
    print(match.group())   # 输出：'123-456-7890'
if match1:
    print(match1.groups())  # 以元组形式返回 ('123', '456', '7890')
```
...
result = re.sub(r'<.*?>|&nbsp','', txt)  #&nbsp 表示html语言中的强制空格
print(result)
```
...
一个实时测试正则表达式是否正确的网站： [https://regex101.com/]，请打开该网站学习使用方法。
...
字典和集合数据结构是 Python 编程的基本构建块。字典（也称为关联数组或映射）是一个无序的键值对集合。它是一个可变的数据结构，允许您使用唯一的键来存储和检索数据。字典由大括号 `{}` 表示，并由逗号分隔的键值对组成。
...
* Json 序列化和去序列化, python中的Json库使用dumps() 和 load()生成和读出json文件。
...
##### 2.5.2 Python 高级数据结构操作（networkx）
...
关于图的算法非常复杂，例如最著名的 *dijkstra* 最短路径算法的应用: 最短路径，迪杰斯特拉算法(Dijkstra)是由荷兰计算机科学家狄杰斯特拉于1959 年提出的，因此又叫狄克斯特拉算法。该算法可以算出从一个顶点到其余各顶点的最短路径，解决的是有权图中最短路径问题。该算法复杂度=$n^2$. 在第7章我们会详细的介绍该经典算法。
...
假设有一个迷宫，其中包含墙壁和通道，您希望找到从起始点到终点的最短路径。迷宫可写为一个矩阵，包括了起点S和终点E，0 表示可以通过的通道，1 表示墙壁。类似于：
...
对以上迷宫使用BFS（广度优先算法）和DFS（深度优先算法）求解迷宫。
**提示：使用deque双端队列输出路径**
