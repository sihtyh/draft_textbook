...
* 通过在不同的数据子集中训练多个模型，Bagging 减减了过拟合，平均出了数据中的噪音。
...
1. **引导采样：** 从训练数据集中随机抽取一个子集，以 replacement 方式。例如，我们可以从训练集中抽取 10,000 条评论。
...
1. **引导采样：** 从训练数据集中随机抽取一个子集，以 replacement 方式。例如，我们可以从整个数据集中抽取 800 天的数据。
